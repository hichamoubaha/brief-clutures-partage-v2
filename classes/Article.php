...
class Article {
    ...
    public function createArticle($titre, $contenu, $id_categorie, $id_auteur, $photo) {
        $query = "INSERT INTO " . $this->table_name . " (titre, contenu, id_categorie, id_auteur, statut, photo) 
                  VALUES (:titre, :contenu, :id_categorie, :id_auteur, 'en attente', :photo)";
    ...
    public function getLatestArticles($page = 1, $limit = 6) {
        $offset = ($page - 1) * $limit;
    ...
    public function getUserArticles($user_id) { // Fixed method name (removed space in 'getUser Articles')
    ...
    public function updateArticle($id, $titre, $contenu, $id_categorie, $photo) {
        $query = "UPDATE " . $this->table_name . " SET titre = :titre, contenu = :contenu, 
                  id_categorie = :id_categorie, photo = :photo WHERE id_article = :id";
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(':id', $id);
        $stmt->bindParam(':titre', $titre);
        $stmt->bindParam(':contenu', $contenu);
    ...
    public function deleteArticle($id, $user_id, $user_role) {
        ...
        if ($article && ($article['id_auteur'] == $user_id || $user_role === 'admin')) {
            ...
            echo "Permission denied or article not found.";
        }
    }
    ...
    public function rejectArticle($id) {
        $query = "UPDATE " . $this->table_name . " SET statut = 'refuse' WHERE id_article = :id"; // Fixed extra space in 'refuse'
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(':id', $id);
        $stmt->execute();
    }
    ...
    public function getArticleWithDetails($id, $is_authenticated = false) {
        ...
        $stmt->bindParam(':id', $id);
        $stmt->execute();
        $article = $stmt->fetch(PDO::FETCH_OBJ); // Fetch as an object
    ...
    public function searchArticles($searchQuery, $page = 1, $limit = 6) {
        $offset = max(0, ($page - 1) * $limit);
        $query = "SELECT a.*, c.nom_categorie, u.nom_utilisateur 
                  FROM " . $this->table_name . " a
                  JOIN categories c ON a.id_categorie = c.id_categorie
                  JOIN utilisateurs u ON a.id_auteur = u.id_utilisateur
                  WHERE a.statut = 'approuve' AND (a.titre LIKE :searchQuery OR a.contenu LIKE :searchQuery)
                  ORDER BY a.date_creation DESC
                  LIMIT :offset, :limit";
        $stmt = $this->conn->prepare($query);
    ...
    public function getTotalSearchResults($searchQuery) {
        $query = "SELECT COUNT(*) as total FROM " . $this->table_name . " 
                  WHERE statut = 'approuve' AND (titre LIKE :searchQuery OR contenu LIKE :searchQuery)";
        $stmt = $this->conn->prepare($query);
        $searchParam = '%' . $searchQuery . '%';
        $stmt->bindParam(':searchQuery', $searchParam, PDO::PARAM_STR);
        $stmt->execute();
